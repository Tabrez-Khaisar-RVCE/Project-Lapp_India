...
def Enter_CD_db(x):
    ...
    mycursor.execute(
        "INSERT INTO Component_Details(Component_Number,Core_Colour,Cable_desc) VALUES(%s,%s,%s)", x)
    db.commit()
...
# Crimp list:
    ...
    mycursor.execute(
        "SELECT * FROM FG_Crimp_Details WHERE FG_Part_Number=%s", (list_fg[1],))
    fg_crp = mycursor.fetchall()
    fg_cr = []
    for i, e in enumerate(fg_crp):
        fg_cr.append(e)
# Assembly list
    for i, e in enumerate(fg_c):
        mycursor.execute(
...
# Insert FG_Assembly:
    for k, v in list_l[5].items():
        for i, e in enumerate(v):
            for j in (1, len(i)+1):
                mycursor.execute("INSERT INTO FG_Assembly(Process,FG_Part_Number,Component_Number,Cable_Sl,Core_Wire_Colors,Testing_Machine_Board_No) VALUES(%s)", (
                    k, list_1[0], list_1[1], list_1[2], e[j], e[0]))
                db.commmit()
...
#     # Details[0]=terminal part number
...
def Delete_WO_db(WO):
    mycursor.execute("DELETE FROM Work_Order_Details WHERE Work_Order_Number=%s", (WO,))
    db.commmit()
...
def Retrieve_Otool_details_show_db(Operation_Type):
    mycursor.execute("Select * FROM Plant_Details WHERE Operation_Type= %s", (Operation_Type,))
    content = mycursor.fetchall()
    return content[0]


def Retrieve_tool_details_show_db([Operation_Type, Machine_tool_id]):
    mycursor.execute("SELECT * FROM Plant_Details WHERE Operation_Type= %s AND Machine_tool_id=%s",
                     (Operation_Type, Machine_tool_id))
    content = mycursor.fetchall()
    return content[0]


def Delete_PD(Machine_tool_id):
    mycursor.execute("DELETE FROM Plant_Details WHERE Machine_tool_id=%s", (Machine_tool_id,))
    db.commmit()


def Modify_PD(Machine_tool_id, x):
    mycursor.execute(
        """UPDATE Plant_Detailss SET
Plant_ID_8261 =%s
Plant_ID_8233 =%s
Plant_ID_8273 =%s
Plant_ID_8235 =%s
Operation_Type =%s
Machine_Type=%s WHERE Machine_tool_id=%s""", (x, Machine_tool_id))
