...
#
# a = input(Enter)
# list = [(962652, "38059480", "LAPP® UNIPLUS 1X70 BK/100mW1 ENDSLV", 11, 0), (962653, "38059220",
                                                                             # "LAPP® ÖLFLEX  110 CY 4G35 /3.5", 1, 0), (962654, "38059220", "LAPP® ÖLFLEX  110 CY 4G35 /3.5", 5, 0)]
...
# nonmatchlist=[[962686, 'KI00316M', '"HA3,SEH,PMB, METALLIC VERSION"', 700, 0], [962687, 'KI00316M', '"HA3,SEH,PMB, METALLIC VERSION"', 600, 0]]

def updatingqty(qty,x):
    mycursor.execute("UPDATE work_order_details SET WO_Qty_Remaining =%s WHERE Work_Order_Number =%s",(qty,x))
    db.commit()

def updatedb(nonmatchlist):
    for x in nonmatchlist:
        mycursor.execute("INSERT INTO Work_Order_Details (Work_Order_Number, FG_Part_Number, Material_Description, WO_Qty_Total, WO_Qty_Remaining) VALUES(%s,%s,%s,%s,%s)", x)
    db.commit()

# updatedb(nonmatchlist)

def commit_WO(list):
    for x in list:
        mycursor.execute(
            "INSERT INTO Work_Order_Details (Work_Order_Number, FG_Part_Number, Material_Description, WO_Qty_Total, WO_Qty_Remaining) VALUES(%s,%s,%s,%s,%s)", x)
        db.commit()
# mycursor.execute("INSERT INTO Work_Order_Details ( Work_Order_Number, FG_Part_Number, Material_Description, WO_Qty_Total, WO_Qty_Remaining) VALUES(%s, %s, %s, %s, %s)",
#            ("962654", '3', 'Gadget', "23456", '123'))
# db.commit()
# mycursor.execute("TRUNCATE Work_Order_Details")
...
#       db.commit()
# mycursor.execute("TRUNCATE Crimping")
# #
# def Commit_UD(list):
# for x in list:
...
# crimpdetails from server database
def crimpdetails():
    mycursor.execute("SELECT * FROM crimping")
    d = mycursor.fetchall()
    # print(d)
    return d

#  FUNCTIONS FOR SERVER DB FOR HTML JINJA
...
# TO GET COLUMN NAMES FROM TABLES DATABASE
# def fetch_col_name():
#     mycursor.execute("SHOW COLUMNS FROM work_order_details")
#     c = mycursor.fetchall()
#     print(c)
# fetch_col_name()
